...
function updateAccount() {
	var msg = XMLGenerator("account", [ "name", "email", "birth_date" ], [
...
function getAccount(parameters) {
	...
	if (request.status == 200) {
		...
		if ($(response).find("response").attr('status') == 'ok') {
			$("#modify_clientname").attr("value",
					$(response).find('name').text());
			$("#modify_datepicker").attr("value",
...
function savePreferences() {
	...
	requestFromServer('SetAccountPreferences', 'username=' + session.username
			+ '&authentication_token=' + session.token + '&value='
	...
	request.onreadystatechange = function() {
		if (request.readyState == 4) {
			if (request.status == 200) {
				var response = request.responseXML;
				if ($(response).find("response").attr('status') == 'ok') {
					setCookie("session", $.toJSON(session), undefined, '/', '',
							false);
					applyPreferences();
				}
			} else {
...
function applyPreferences() {
	if (session.preferences.theme == $ORANGE) {
		...
		$("#headerContent").css("background-image",
				"url(../css/images/inicio/SilverTheme/header_test.png)");
